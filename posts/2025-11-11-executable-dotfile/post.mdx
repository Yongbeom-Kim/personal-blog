...
description: "Building a self-installing, declarative dotfile management system using executable scripts."
unlisted: true
"og:image": executable-vimrc-preview.png
---
...
Of course, my next thought was to _replicate_ this setup across other devices,
...
Fundamentaly, a dotfile manager must keep two pieces of information in sync for every configuration file: the **contents**, and **location** of the dotfile.
...
Coming back to the repository after an entire year, I didnt't think this was a good design.
...
In 2023, I was sold the idea that symlinks were the holy grail of dotfile management.
Create a symlink `$HOME/.vimrc  ⭢ $GIT_ROOT/dotfiles/vim/.vimrc`, and all updates are synced to your repo for version control.

I've since changed my mind.

My work-issued laptop has ended up having some work-specific configs in my `.zshrc`
that I would prefer not to be synced across all my machines, and I may find more such cases as well.
Sure, they share a kind of **base dotfile**, but each also has its own machine-specific configs.

Additionally, configuration files are _sensitive_.
For me, the invisible and automatic way in which symlinks propagate dotfile changes to and fro from version control is bad, and should be avoided.

What happens if you do a `git pull` in your dotfile repo and a Linux-specific configuration file breaks your MacOS environment?

Any actions involving dotfiles should be **explicit** and intentional.

Quick background before I go over my new design:

## About `#!` Shebangs

Feel free to skip this section if you know how shebangs work.
...
This prints the file back at you:
...
My new dotfile system is predicated on this idea.

### Initial Design & Challenges

Initially, the design looked something like this:
...
Turns out, MacOS's shebang parser doesn't allow you to escape spaces.
It seems like the parser uses a simple whitespace-based tokenizer that
doesn't respect shell quoting rules, so you can put double quotes, single quotes,
or escape the space, but they will always be resolved as separate arguments.

To say the least, this limitation wasn't something I was happy with.
...
## The Second (Hopefully Last) Rewrite
...
We have the _dotfiles_ git repo, and we can think of the dotfiles in the relevant paths in the system as a remote origin.
...
Many hours were spent writing, testing and debugging bash scripts. But, this approach is one that I am finally happy with (at least until the next rewrite).

Each dotfile is self-contained, declarative and executable, with no magic abstractions.

This *executable dotfile* model keeps everything explicit and intentional, because configuration files just feel so sensitive for me.