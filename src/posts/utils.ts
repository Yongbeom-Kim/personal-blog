...
export const getSlug = (frontmatter: PostFrontmatter) => {
  return frontmatter.date.toISOString().split("T")[0] + "-" + frontmatter["slug-suffix"];
}
...
export const importAllPosts = async (): Promise<Map<string, Post>> => {
  ...
  const postModules = import.meta.glob("/src/posts/**/*.mdx", {
    ...
    string,
    { frontmatter: PostFrontmatter; Content: React.ComponentType }
  >;
  ...
  Object.values(postModules).forEach((module) => {
    ...
    const slug = getSlug(post.frontmatter)
